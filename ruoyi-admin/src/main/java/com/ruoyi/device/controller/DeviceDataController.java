...
public class DeviceDataController extends BaseController
{
     ...
     */
    @PreAuthorize("@ss.hasPermi('device:device:list')")
    @GetMapping("/listByRoleAndDept")
    public TableDataInfo listByRoleAndDept(@RequestParam String roleName, @RequestParam String deptName, DeviceData deviceData)
    {
        startPage();
        List<DeviceData> list = deviceDataService.selectDeviceDataByRoleAndDept(roleName, deptName, deviceData);
        return getDataTable(list);
    }
    /**
     * 导出设备列表列表
